...
export default class MultiSelect extends Component {
  ...
  _displaySelectedItems = (optionalSelctedItems) => {
    ...
    return actualSelectedItems.map((singleSelectedItem) => {
      ...
      return (
        <View style={[styles.selectedItem, {width: (item[displayKey].length * 8) + 60, justifyContent: 'center', height: 40, borderColor: tagBorderColor}]}
          key={item[uniqueKey]}>
          <Text style={[{flex: 1, color: tagTextColor, fontSize: 14}, fontFamily ? { fontFamily } : {}]} numberOfLines={1} >
            {item[displayKey]}
          </Text>
          <TouchableOpacity onPress={() => {this._removeItem(item);}}>
            <Icon name="times" style={{ color: tagRemoveIconColor, fontSize: 20, marginLeft: 10, marginRight: 10 }}/>
          </TouchableOpacity>
        </View>
      );
    });
          ...
          <View >
            <View style={styles.select_box}>
              <TextInput
                ...
                style={[searchInputStyle, { flex: 1 }, styles.select_box_input]}
                value={searchTerm}
              />
              {hideSubmitButton && (
                <TouchableOpacity onPress={this._submitSelection} >
                  <Icon name="times" style={[styles.select_box_icon]} size={20}/>
                </TouchableOpacity>
              )}
            </View>
            <View style={[{ flexDirection: 'column'}, styles.drop]}>
            ...
            <View style={styles.dropdownView}>
              <View style={[styles.subSection]}>
                <TouchableWithoutFeedback onPress={this._toggleSelector}>
                  <View style={[styles.select_box, {padding: 15}]}>
                    <Text
                      style={[{ flex: 1, fontSize: fontSize || 16, color: textColor || colorPack.placeholderTextColor},
                        altFontFamily? { fontFamily: altFontFamily } : fontFamily ? { fontFamily } : {}]}
                      numberOfLines={1}>
                      {this._getSelectLabel()}
                    </Text>
                    <Icon name={'search'} size={20} style={styles.select_box_icon}/>
                  </View>
                </TouchableWithoutFeedback>
              </View>
            </View>
